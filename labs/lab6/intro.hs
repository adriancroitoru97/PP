...
computeLength getLineSegment getStartPoint getEndPoint =
    let segment =    getLineSegment
        ...
        (x1, y1) =   (fst start, snd start)
        (x2, y2) =   (fst stop, snd stop)
...
mergeSort lst = let computeHalf = (\f lst -> (f (quot (length lst) 2) lst))
                ...
                in if null lst
                    then []
                        ...
                        then lst
                        else (merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst)))

mergeSort2 :: [Int] -> [Int]
mergeSort2 lst
    | null lst = []
    | null (tail lst) = lst
    | otherwise = (merge (mergeSort2 (fstHalf lst)) (mergeSort2 (sndHalf lst)))
    where
