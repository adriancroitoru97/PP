...
insertElem tree@(BSTNode k l r) x = if x < k then BSTNode k (insertElem l x) r else BSTNode k l (insertElem r x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem tree@(BSTNode k l r) x
  | x == k = Just x
  | x < k = findElem l x
  | otherwise = findElem r x

inorder :: BST a -> [a]
inorder BSTNil = []
inorder oneNodeTree@(BSTNode x BSTNil BSTNil) = [x]
inorder tree@(BSTNode k l r) = inorder l ++ [k] ++ inorder r

size :: (BST a) -> Int
size BSTNil = 0
size tree@(BSTNode k l r) = 1 + size l + size r

height :: (BST a) -> Int
height BSTNil = 0
height tree@(BSTNode k l r) =
  let leftHeight = height l
      rightHeight = height r
  in if leftHeight > rightHeight then leftHeight + 1 else rightHeight + 1

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced tree@(BSTNode k l r) =
  let leftHeight = height l
      rightHeight = height r
  in isBalanced l && isBalanced r && (abs (leftHeight - rightHeight) <= 1)
