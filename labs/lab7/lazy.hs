...
newtonRaphsonSolve x_old atol g dg = 
    let stream = (newtonRaphson x_old g dg)
        pairs = (zip stream (tail stream))
    in (fst $ head $ dropWhile (\x -> (abs ((fst x) - (snd x))) > atol) pairs)
...
babylonianMethod a x_old atol = newtonRaphsonSolve x_old atol (\x -> x * x - a) (\x -> 2 * x)
