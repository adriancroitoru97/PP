...
instance Eq a => Eq (BST a) where
    (==) BSTNil BSTNil = True
    (==) (BSTNod v1 l1 r1) (BSTNod v2 l2 r2) = (v1 == v2) && (l1 == l2) && (r1 == r2)
    (==) _ _ = False
...
-}
repeatTabs :: Int -> [Char]
repeatTabs idx = if idx == 0 then "" else "\t" ++ repeatTabs (idx - 1)

showHelper :: (Show a) => BST a -> Int -> [Char]
showHelper BSTNil idx = ""
showHelper (BSTNod v l r) idx = repeatTabs idx ++ show v ++ "\n" ++ showHelper l (idx + 1) ++ showHelper r (idx + 1)
instance Show a => Show (BST a) where
    show x = showHelper x 0
...
instance Invertible (BST a) where
    ...
    invert (BSTNod v l r) =  BSTNod v (invert l) (invert r)
...
instance Foldable BST where
    ...
    foldr f acc (BSTNod v l r) = foldr f (f v (foldr f acc r)) l
...
instance Container BST where
    contents tree = foldr (\x acc -> acc ++ [x]) [] tree
