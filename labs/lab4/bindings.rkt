...
(define (compute-length get-line-segment get-start-point get-end-point)
  ...
  (sqrt (+ (sqr (- (car end) (car start))) (sqr (- (cdr end) (cdr start)))))))
...
(define (distance x y)
  (let* ([segment (λ () (cons x y))]
         [first (λ (segment) (car segment))]
         [second (λ (segment) (cdr segment))])
  (compute-length segment first second)))
...
(define (compute-perimeter points)
  (let compute-rec ([per 0] [p points] [first (car points)])
    (if (= (length p) 1)
        (+ per (distance first (car p)))
        (compute-rec (+ per (distance (car p) (cadr p))) (cdr p) first))))
...
(define (3-sequence-max numbers separator)
  (let-values ([(first-seq rest1) (splitf-at numbers (λ (x) (if (not (= x separator)) #t #f)))])
    (let-values ([(second-seq rest2) (splitf-at (cdr rest1) (λ (x) (if (not (= x separator)) #t #f)))])
      (let ([s1 (sum first-seq)]
            [s2 (sum second-seq)]
            [s3 (sum (cdr rest2))])
        (if (and (> s1 s2) (> s1 s3))
            s1
...
(define (list-num-concat numbers)
  (string->number (foldl (λ (x acc) (string-append acc (number->string x))) "" numbers)))
...
(define (find-all-suffixes number)
  (let suffixes ([nr (number->list number)])
    (if (null? nr)
        null
        (cons (list-num-concat nr) (suffixes (cdr nr))))))
...
(define (run initial-state final-state next)
  (let run-let ([actual initial-state])
    (if (= actual final-state)
        (list actual)
        (cons actual (run-let (next actual))))))
...
(define (generate-number k x)
  (let ([initial x]
        [final (+ x (* k (- k 1)))]
        [step (λ (x) (+ x k))])
    (list-num-concat (run initial final step))))
