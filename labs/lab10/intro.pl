...
myReverse([], []).
myReverse([H1 | T1], RevList) :- myReverse(T1, RevTemp), myConcat(RevTemp, [H1], RevList).
...
myReverseAcc([], RevListAcc, RevListAcc).
myReverseAcc([H | T], Acc, RevListAcc) :- myReverseAcc(T, [H | Acc], RevListAcc).
...
sameTree(X, Y):- descendantOf(X, Z), descendantOf(Y, Z).
...
drum(X, X, [X]).
drum(X, Y, [X, Y]) :- arc(X, Y).
drum(X, Y, [X | T]) :- descendantOf(Z, X), descendantOf(Y, Z), drum(Z, Y, T).
drum(X, Y, [X | T]) :- descendantOf(X, Z), drum(Z, Y, T).
...
cost(X, Y, 1) :- arc(X, Y).
cost(X, Y, -1) :- arc(Y, X).
cost(X, Y, N) :- arc(Z, Y), descendantOf(Z, X), cost(Z, Y, N1), N1 is N - 1.
cost(X, Y, N) :- arc(X, Z), cost(Z, Y, N1), N1 is N + 1.
