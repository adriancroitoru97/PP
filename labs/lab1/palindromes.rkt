...
(define (all-palindromes? n Bases)
  (cond
    [(null? Bases) #t]
    [(and (palindrome? (num->base n (car Bases))) (all-palindromes? n (cdr Bases))) #t]
    [else #f]))
...
(define (nr-pal start b)
  (cond
    [(= b 1) null]
    [(palindrome? (num->base start b)) (append (nr-pal start (- b 1)) (list b))]
    [else (nr-pal start (- b 1))]))
    

(define (pal-atlb? start b)
  (if (>= (length (nr-pal start 10)) b) #t #f))


(define (first-b-pal start b)
  (cond
    [(pal-atlb? start b) start]
...
(define (longest-palindrome n)
   (if (palindrome? (num->base n 10))
       ...
       (if  (>
            (length (num->base (longest-palindrome (list->num (take (num->base n 10) (- (length(num->base n 10)) 1)))) 10))
            (length (num->base (longest-palindrome (list->num (take (cdr(num->base n 10)) (- (length(cdr(num->base n 10))) 1)))) 10)))
            (longest-palindrome (list->num (take (num->base n 10) (- (length(num->base n 10)) 1))))
            (longest-palindrome (list->num (take (cdr(num->base n 10)) (length(cdr(num->base n 10))) ))))))
