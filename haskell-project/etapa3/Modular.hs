...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set = isModuleHelper set set

isModuleHelper :: Ord a => S.Set a -> S.Set a -> Graph a -> Bool
isModuleHelper set initial graph =
    S.size set == 0 || S.size set == 1 ||
    let currentIn = S.filter (`notElem` initial) (inNeighbors (head (S.toList set)) graph)
        currentOut = S.filter (`notElem` initial) (outNeighbors (head (S.toList set)) graph)
        nextIn = S.filter (`notElem` initial) (inNeighbors (head (tail(S.toList set))) graph)
        nextOut = S.filter (`notElem` initial) (outNeighbors (head (tail(S.toList set))) graph)
    in currentIn == nextIn && currentOut == nextOut && isModuleHelper (S.drop 1 set) initial graph
{-
    *** TODO ***
...
isModularPartition partition graph =
    (S.size partition == 0) ||
    (isModule (head (S.toList partition)) graph && isModularPartition (S.drop 1 partition) graph)
...
maximalModularPartition partitions graph =
    minimumBy (compare `on` (\x ->
                                if S.size x == 1 || not (isModularPartition x graph)
                                    then 10000
                                else S.size x)
                        ) partitions
